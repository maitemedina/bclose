...
class EditUser_Page extends StatefulWidget {
  ...
  const EditUser_Page({Key? key, required this.position}) : super(key: key);
...
class _EditUser_Page extends State<EditUser_Page> {
  ...
  void initState() {
    ...
    super.initState();
    setState(() {
      userName.text = sOSController.userSos[widget.position].name;
      numeber.text =  sOSController.userSos[widget.position].number;
      morada.text =  sOSController.userSos[widget.position].morada;
    });
  ...
  _onSave(String command) {
    setState(() {
      if(userName.text!=""&&numeber.text!=""){
        ...
        sOSController.UpateUser(base64Image,morada.text,userName.text,numeber.text,sOSController.userSos[widget.position].id.toString(),context);
      }else{
  ...
  _onRemove(String command) {
    Get.defaultDialog(
      textConfirm: 'Sim',
      title: 'Remover o contacto',
      onConfirm: () {
        sOSController.RemoveUser(sOSController.userSos[widget.position].id.toString(),context);
      },
